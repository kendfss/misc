...
if __name__ == '__main__':
    ...
    # for i in range(1,10):
        ...
        # D = mine(start,1,True)
        # print(i,start,D,sep='\t')
        # print(i,strong.startswith(str(D)),start,D,sep='\t')
        # tabulate([i,start,D])
    
    # with open('digiparsefatbastard.txt', 'w') as file:
        # file.write(tabulate(((i,strong.startswith(str(mine(first(strong,i),2,True)))) for i in range(1,len(strong)//2)),headers=['i','D in long']))
    with open('digiparsefatbastard.txt', 'r') as file:
        for line in file.readlines():
            if re.search('True',line):
                print(line)
    # for i in range(1,len(strong)//2):
        # if 
