...
Todo:
    implement slicing
    check if there is a cross product for sets
"""
...
from sl4ng import show, main, generator, getsource

def regurge_(iterable:Iterable[Any]) -> Iterable:
    """
    No longer
...
def pascals(rows=-1):
    ...
    while rows:
        yield last
        last = (1, *(sum(last[i-1:i+1]) for i in range(1, len(last))), 1)
        rows -= 1

def pascal(*indices):
    yield from _regen.choose(pascals(), *indices)
...
class regenerator:
    ...
    def inject(self, value):
        ...
        self._inert = chain(self._inert, *other)
        return self
regurge = regen = regenerator

def length(iterable):
    return sum(1 for i in regurge(iterable))
...
if eval(main):
    ...
    print('\n'*3)
    print([*map(getsizeof, (w, x, y))])
    print([eval(f'{i}.__sizeof__()') for i in 'wxy'])
    print()